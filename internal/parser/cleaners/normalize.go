...
func productNormalizer(name, category string) string {
	...
	case "notebooks":
		if brand == "acer" {
			// A16-52M-78VR 			-> A16-52M
			// TMB311-34-TCO-C32T 		-> TMB311-34
			// TMP215-55-G2-TCO-7047 	-> TMP215-55-G2-TCO
			name = regexp.MustCompile(`\(?([A-Z]{1,3}[0-9]{2,3}(?:[A-Z]{1,2})?(?:-[0-9]{2,3}[A-Z]*))\)?`).ReplaceAllString(name, "($1)")
			name = regexp.MustCompile(`\)-(.*?)\).*`).ReplaceAllString(name, ")") // remove suffixes after model number in brackets
		}
		if brand == "apple" {
			// Apple MacBook Pro 14 M5 2025 10C CPU/10C GPU
			name = regexp.MustCompile(`\s(\d{2})\s`).ReplaceAllString(name, " $1\" ")

			// 15" 2025 M4 				-> (15", M4, 2025)
			// 14" M5 2025 				-> (14", M5, 2025)
			...
			// 13 MacBook Neo (A18 Pro) -> (13", A18 Pro)
			if inch := regexp.MustCompile(`\s(\d{2})"?\s`).FindStringSubmatch(name); inch != nil {
				name = strings.ReplaceAll(name, inch[0], " ") // remove inch from name
		...
		if brand == "hp" {
			name = regexp.MustCompile(`(?i)(\s+Next\s+Gen\s+)?(AI\s+PC).*`).ReplaceAllString(name, "")
		}
	}
